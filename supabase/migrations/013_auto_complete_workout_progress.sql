...
begin
  ...
  synced_completed_at := case when synced_status = 'completed' then clock_timestamp() else null end;
...
where session.workout_plan_id = plan.id
  and session.status in ('partial', 'completed');

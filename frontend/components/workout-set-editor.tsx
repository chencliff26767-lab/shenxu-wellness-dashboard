  ...
  return (
    ...
    >
      ...
      <div className={`flex items-end gap-1.5 ${completed ? "opacity-60" : ""}`}>
        ...
        <button aria-label={`儲存第 ${set.position} 組`} className="flex h-11 w-9 shrink-0 items-center justify-center rounded-md border border-border text-muted-foreground disabled:opacity-40" disabled={pending} type="submit">
      ...
      {undoValue != null ? (
        <button className="mt-1 flex min-h-9 items-center gap-1 text-xs text-muted-foreground underline underline-offset-4" onClick={undo} type="button">
      ...
      <details className="mt-1">
        <summary className="min-h-9 cursor-pointer py-2 text-xs text-muted-foreground">組別備註</summary>
...
function SetInput({ label, max, min = "0", name, onChange, planned, step = "0.5", value }: { label: string; max?: string; min?: string; name: string; onChange: () => void; planned: number | string | null; step?: string; value: number | string | null }) {
  return (
    <label className="w-14 shrink-0 text-center text-[10px] leading-none text-muted-foreground">
      {label}
      <input aria-label={label} className="mt-1 h-9 w-full rounded-md border border-border bg-card px-1 text-center text-sm text-foreground outline-none focus:ring-2 focus:ring-primary" defaultValue={value ?? planned ?? ""} inputMode="decimal" max={max} min={min} name={name} onChange={onChange} placeholder="—" step={step} type="number" />
    </label>
  );
}
